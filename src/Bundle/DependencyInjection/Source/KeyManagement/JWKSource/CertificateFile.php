...
final readonly class Certificate_File extends Abstract_Source implements Jwk_Source
{
    /**
    ...
    public function create_definition(Container_Builder $container, array $config): Definition
    {
        ...
        $definition->set_factory([new Reference(Jwk_Factory::class), 'createFromCertificateFile']);
        $definition->set_arguments([$config['path'], $config['additional_values']]);
    ...
    public function add_configuration(Node_Definition $node): void
    {
        ...
        $node->children()->scalar_node('path')->info('Path of the certificate file.')->is_required()->end()->array_node('additional_values')->info('Additional values to be added to the key.')->default_value([])->use_attribute_as_key('key')->variable_prototype()->end()->end()->end();
    }
}