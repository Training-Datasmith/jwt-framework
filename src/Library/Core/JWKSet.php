...
use Traversable;
class Jwk_Set implements Countable, IteratorAggregate, JsonSerializable
{
    private array $keys = [];
    /**
     * @param JWK[] $keys
     ...
     */
    public static function create_from_key_data(array $data): self
    {
        if (!isset($data['keys'])) {
    ...
    public static function create_from_json(string $json): self
    {
        ...
        return self::create_from_key_data($data);
    }
    /**
     ...
     */
    public function select_key(string $type, ?Algorithm $algorithm = null, array $restrictions = []): ?JWK
    {
        ...
        foreach ($this->keys as $key) {
            $ind = 0;
            $can_use = $this->can_key_be_used_for($type, $key);
            if ($can_use === false) {
                continue;
            }
            $ind += $can_use;
            $alg = $this->can_key_be_used_with_algorithm($algorithm, $key);
            if ($alg === false) {
                continue;
            }
            $ind += $alg;
            if ($this->does_key_satisfy_restrictions($restrictions, $key) === false) {
                continue;
            }
     ...
     * @internal
     */
    public static function sort_keys(array $a, array $b): int
    {
        return $b['ind'] <=> $a['ind'];
    }
    /**
    ...
    private function can_key_be_used_for(string $type, JWK $key): bool|int
    {
        if ($key->has('use')) {
        ...
        if ($key->has('key_ops')) {
            ...
            return $type === self::convert_key_ops_to_key_use($key_ops) ? 1 : false;
        }
        return 0;
    }
    private function can_key_be_used_with_algorithm(?Algorithm $algorithm, JWK $key): bool|int
    {
        if ($algorithm === null) {
            return 0;
        }
        if (!in_array($key->get('kty'), $algorithm->allowed_key_types(), true)) {
            return false;
        }
        if ($key->has('alg')) {
    ...
    private function does_key_satisfy_restrictions(array $restrictions, JWK $key): bool
    {
    ...
    private static function convert_key_ops_to_key_use(array $key_ops): string
    {
        return match (true) {
