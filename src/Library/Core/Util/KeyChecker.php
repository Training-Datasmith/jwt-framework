 ...
 * @internal
 */
final readonly class Key_Checker
{
    public static function check_key_usage(JWK $key, string $usage): void
    {
        if ($key->has('use')) {
            self::check_usage($key, $usage);
        }
        if ($key->has('key_ops')) {
            self::check_operation($key, $usage);
        }
    }
    public static function check_key_algorithm(JWK $key, string $algorithm): void
    {
        if (!$key->has('alg')) {
            return;
        }
    ...
    private static function check_operation(JWK $key, string $usage): void
    {
        $ops = $key->get('key_ops');
        if (!is_array($ops)) {
    ...
    private static function check_usage(JWK $key, string $usage): void
    {
        $use = $key->get('use');
        switch ($usage) {
            case 'verification':
