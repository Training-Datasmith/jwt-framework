 ...
 */
final readonly class Not_Before_Checker implements Claim_Checker, Header_Checker
{
    ...
    public function check_claim(mixed $value): void
    {
        if (!is_float($value) && !is_int($value)) {
            throw new Invalid_Claim_Exception('"nbf" must be an integer.', self::NAME, $value);
        }
        $now = $this->clock->now()->get_timestamp();
        if ($now < $value - $this->allowed_time_drift) {
            throw new Invalid_Claim_Exception('The JWT can not be used yet.', self::NAME, $value);
        }
    }
    #[Override]
    ...
    {
        if (!is_float($value) && !is_int($value)) {
            throw new Invalid_Header_Exception('"nbf" must be an integer.', self::NAME, $value);
        }
        $now = $this->clock->now()->get_timestamp();
        if ($now < $value - $this->allowed_time_drift) {
            throw new Invalid_Header_Exception('The JWT can not be used yet.', self::NAME, $value);
        }
    }
    #[Override]
