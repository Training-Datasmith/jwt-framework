...
use Override;
abstract readonly class ECDHESAESKW extends Abstract_Ecdhaeskw
{
    /**
    ...
    {
        $ecdh_es = new ECDHES();
        $agreement_key = $ecdh_es->get_agreement_key($this->get_key_length(), $this->name(), $recipient_key->to_public(), $sender_key, $complete_header, $additional_header_values);
    ...
    {
        $ecdh_es = new ECDHES();
        $agreement_key = $ecdh_es->get_agreement_key($this->get_key_length(), $this->name(), $recipient_key, $sender_key, $complete_header);
        $wrapper = $this->get_wrapper();
